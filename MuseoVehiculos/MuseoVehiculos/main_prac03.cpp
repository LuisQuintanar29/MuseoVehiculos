...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (float i = 1; i < 3; i++)
		{
			...
			myShader.setMat4("model", modelOp);
			glDrawArrays(GL_TRIANGLES, 0, 36); // Cube right

			modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(-i, 0.0f, 0.0f));
			myShader.setMat4("model", modelOp);
			glDrawArrays(GL_TRIANGLES, 0, 36); // Cube left
		}
		...
		myShader.setVec3("aColor", glm::vec3(0.65f, 0.5f, 0.39f));
		myShader.setMat4("model", modelOp);
		glDrawArrays(GL_TRIANGLES, 0, 36); //My Cube
		...
		myShader.setVec3("aColor", glm::vec3(1.0f, 0.5f, 0.0f));
			modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(5, 2.0f, 0.0f));
			myShader.setMat4("model", modelOp);
			glDrawArrays(GL_TRIANGLES, 0, 36); // Cube right
...
void my_input(GLFWwindow *window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)  //GLFW_RELEASE
		movX += 0.1f;
	if (glfwGetKey(window, GLFW_KEY_PAGE_DOWN) == GLFW_PRESS)  //GLFW_RELEASE
		movY-= 0.1f;
	if (glfwGetKey(window, GLFW_KEY_PAGE_UP) == GLFW_PRESS)  //GLFW_RELEASE
		movY += 0.1f;
